...
public class AfkCommand {
        ...
        public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
                dispatcher.register(literal("afk")
                                .requires(src -> Permissions.check(src, "antilogout.command.afk",
                                                config.afk.permissionLevel))
                                .then(literal("players")
                                                .requires(src -> Permissions.check(src,
                                                                "antilogout.command.afk.players",
                                                                config.afk.permissionLevel))
                                                .then(Commands.argument("targets", EntityArgument.players())
                                                                .then(literal("time")
                                                                                .requires(src -> Permissions.check(src,
                                                                                                "antilogout.command.afk.players.time",
                                                                                                config.afk.permissionLevel))
                                                                                .then(Commands.argument("time",
                                                                                                DoubleArgumentType
                                                                                                                .doubleArg(5, config.afk.maxAfkTime == -1
                                                                                                                                ? Double.MAX_VALUE
                                                                                                                                : config.afk.maxAfkTime))
                                                                                                .executes(ctx -> AfkCommand
                                                                                                                .afkPlayers(EntityArgument
                                                                                                                                .getPlayers(ctx, "targets"),
                                                                                                                                DoubleArgumentType
                                                                                                                                                .getDouble(ctx, "time")))))
                                                                .executes(ctx -> AfkCommand.afkPlayers(
                                                                                EntityArgument.getPlayers(ctx,
                                                                                                "targets"),
                                                                                config.afk.maxAfkTime))))

                                .then(literal("time")
                                                .requires(src -> Permissions.check(src, "antilogout.command.afk.time",
                                                                config.afk.permissionLevel))
                                                .then(Commands.argument("time", DoubleArgumentType.doubleArg(5,
                                                                config.afk.maxAfkTime == -1 ? Double.MAX_VALUE
                                                                                : config.afk.maxAfkTime))
                                                                .executes(ctx -> AfkCommand.afkPlayers(
